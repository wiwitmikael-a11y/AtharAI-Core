...
import { MODE_DETAILS, MODEL_API_IDS } from './constants';
...
const ModelStatusIndicator: React.FC<{ status: 'online' | 'loading' | 'offline' | 'unknown' }> = ({ status }) => {
    const statusConfig = {
        online: { color: 'bg-green-400', title: 'Model is Online' },
        loading: { color: 'bg-yellow-400 animate-pulse', title: 'Model is Loading...' },
        offline: { color: 'bg-red-500', title: 'Model is Offline' },
        unknown: { color: 'bg-slate-500', title: 'Model status is unknown' },
    };
    const { color, title } = statusConfig[status];
    return <span className={`w-2.5 h-2.5 rounded-full ${color}`} title={title}></span>;
};
...
const App: React.FC = () => {
    ...
    const { activeMode, setActiveMode, modelStatuses } = context;
      ...
      <div className="flex flex-col h-full">
        ...
        <nav className="flex-1 space-y-2">
            {(Object.keys(ChatMode) as Array<keyof typeof ChatMode>).map((key) => {
                ...
                const modelId = MODEL_API_IDS[mode];
                const status = modelId ? modelStatuses[modelId] || 'unknown' : 'online'; // Assume 'online' for local modes like To-Do
                ...
                return (
                    ...
                    >
                        ...
                        <span className="font-medium flex-1">{MODE_DETAILS[mode].name}</span>
                        {mode !== ChatMode.Todo && <ModelStatusIndicator status={status} />}
                    </button>
                );
            })}
        </nav>
